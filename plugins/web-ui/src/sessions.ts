...
function statusMarks(s: CoreSession): TemplateResult {
  ...
  const inspect = `${ind.background?.label ?? ""} — ${t("click to inspect")}`;
  return html`${ind.working ? html`<span class="working-dot" ${ref(syncWorkingPulse)} title=${t("Agent is working")} aria-label=${t("Agent is working")}></span>` : nothing}${
    ...
    ind.background
      ? html`<span
          ...
          tabindex="0"
          aria-label=${inspect}
          @mouseenter=${(e: Event) => showTooltip(e.currentTarget as Element, inspect)}
          @mouseleave=${(e: Event) => hideTooltip(e.currentTarget as Element)}
          @focus=${(e: Event) => showTooltip(e.currentTarget as Element, inspect)}
        ...
        s.id
          ? html`<span class="chat-row-actions">
              <button
                ...
                type="button"
                title=${t("Copy link")}
                aria-label=${t("Copy link to {name}", { name: sessionTitle(s) })}
              ...
              <button
                ...
                aria-label=${t(s.pinned ? "Unpin {name}" : "Pin {name}", { name: sessionTitle(s) })}
                @click=${() => {
              ...
              <button
                ...
                aria-label=${t(s.archived ? "Unarchive {name}" : "Archive {name}", { name: sessionTitle(s) })}
                @click=${() => {
...
function sessionRow(s: CoreSession, projectChild = false): TemplateResult {
  ...
  const ariaLabel = [
    ...
    context,
    working ? t("Agent is working") : null,
    s.awaitingInput ? t("waiting for your reply") : null,
    readOnly ? t("read-only") : null,
    s.pinned ? t("pinned") : null,
...
export async function openSessionInto(
  ...
  if (!entriesRes) {
    if (tracked) showMainEmpty(t("Couldn't load this conversation. Check your connection and click it again."));
    renderList();
    return;
  }
