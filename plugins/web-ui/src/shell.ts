...
function impersonationBanner(by: string) {
  return html`
    <div class="top-banner" role="status">
      <span>${t("Viewing the assistant as {who}", { who: appState.me?.user ?? "" })} — ${t("you are {who}", { who: by })}</span>
...
function devBanner(user: string) {
  return html`
    <div class="top-banner dev" role="status">
      <span>${t("Dev mode")} — ${t("no identity provider, signed in as {user}", { user })}</span>
      ...
      <div class="layout ${sidebarOpen ? "" : "sidebar-closed"} ${banner ? "bannered" : ""}">
        <aside class="sidebar" aria-label=${t("Navigation")} @keydown=${onSidebarKeydown}>
          ...
          <div class="sidebar-footer">
            ...
            <a class="icon-btn subtle" href=${ADMIN_HOME_URL} title=${t("Back to admin")} aria-label=${t("Back to admin")}
              >${icon(ArrowLeft, 17)}</a
            >
...
export function renderSidebarTop(): void {
  ...
  const navGroup = (id: string, title: string, open: boolean, toggle: () => void, rows: TemplateResult) => html`
    <button
      ...
      aria-controls=${id}
      title=${t(open ? "Hide {title}" : "Show {title}", { title })}
      @click=${toggle}
    >
      <span>${title}</span>
  ...
  render(
    html`
      <button
        class="new-chat"
        title=${splitState.active ? t("New session") : t("New chat")}
        @click=${() => {
          closeSidebarOnNarrowView();
          if (!addBlankPane()) mainConversation().newChat();
        }}
      >
        ${icon(ICON.newChat, 17)}<span>${splitState.active ? t("New session") : t("New chat")}</span>
      </button>
...
export function renderPane(
  ...
  const refreshLabel = t("Refresh {title}", { title: title.toLowerCase() });
