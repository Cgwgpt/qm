...
export async function renderContexts(): Promise<void> {
  ...
  try {
    ...
    if (seq !== appState.viewRenderSeq || appState.currentView !== "contexts") return;
    contextsNotice = errMessage(e, t("Failed to load contexts."));
  }
...
function contextMeta(c: CoreContext): { title: string; sub: string; glyph: IconNode } {
  if (c.project) {
    ...
    return {
      ...
      sub: t("{n} member{s}", { n: memberCount, s: memberCount === 1 ? "" : "s" }),
      glyph: Folder,
    };
  }
  if (c.kind === "personal") {
    return { title: t("Personal"), sub: t("Just you — your web chats and DMs with the agent live here."), glyph: User };
  }
  if (c.kind === "group") {
    return {
      title: sharedContextLabel(c.scopeId, c.name) ?? t("Group DM"),
      sub: t("Shared with everyone in this group conversation."),
      glyph: Users,
    };
  }
  return {
    title: sharedContextLabel(c.scopeId, c.name) ?? t("Channel"),
    sub: t("Shared with everyone in this channel."),
    glyph: Hash,
  };
}
...
export function scopeChip(scopeId: string | null, fallbackName?: string | null): TemplateResult {
  ...
  return html`<span class="scope-chip" title=${t("In {title}", { title })}
  ...
  return html`
    <div class="menu-control form-menu-control scope-filter">
      <button class="menu-button" type="button" aria-haspopup="menu" aria-expanded="false" @click=${toggleFormMenu}>
        ${icon(ListFilter, 14)}<span class="menu-label">${t("Filter by: {label}", { label })}</span>${icon(ChevronDown, 14)}
      </button>
      <div class="menu-popover" role="menu" hidden>
        <div class="menu-title">${t("Filter by context")}</div>
...
function gridTpl(): TemplateResult {
  const status =
    contextsNotice || (contextsLoading && contextsState.list.length === 0 ? t("Loading projects…") : "");
  ...
  if (projects.length) projectList = html`<div class="grid project-grid">${projects.map(contextCard)}</div>`;
    ...
    projectList = html`<div class="empty compact project-empty">
      ${projectsFiltered ? t("No projects match your search.") : t("No projects yet.")}
    </div>`;
  }
  return html`
    <div class="project-grid-content">
      <div class="pane-head">
        <h1 class="pane-title">${t("Projects")}</h1>
        <div class="project-head-actions">
          ...
          >
            ${icon(FolderPlus, 15)}<span>${t("New project")}</span>
          </button>
        </div>
      </div>
  ...
  return html`
    <button class="card context-card" type="button" @click=${() => selectContext(c.scopeId)}>
      <div class="card-head">
        ...
        ${c.isPrivate ? html`<span class="context-lock" title=${t("Private channel")}>${icon(Lock, 13)}</span>` : nothing}
        <span class="badge">${t(access)}</span>
      </div>
      <div class="card-meta context-card-sub">${sub}</div>
      <div class="card-meta">${count}${c.lastActivityAt ? ` · ${t("active {n}", { n: relTime(c.lastActivityAt) })}` : ""}</div>
    </button>
  `;
}
  ...
  return html`
    <div class="context-detail">
      ...
      <div class="context-detail-head">
        ...
        <div class="context-detail-titles">
          ...
          <div class="context-sub">
            ${c.project
              ? sub
              : `${sub} ${t("The agent's files and memory here are separate from your other contexts.")}`}
          </div>
        </div>
      ...
      <div class="context-workspace has-settings">
        ...
        <aside class="context-settings" aria-label=${c.project ? t("Project settings") : t("Context settings")}>
        ...
        ${projectPeople(context).map((principalId) => {
          ...
          return html`
            <div class="project-member-row">
              ...
              ${principalId === project.ownerId ? html`<span class="badge">${t("Owner")}</span>` : nothing}
              ${
...
function memberPicker(context: CoreContext): TemplateResult {
  ...
  if (idle && contextsState.memberSearchedQuery && matches.length === 0) {
    emptyNote = contextsState.memberMatches.length
      ? t("Everyone matching is already in this project.")
      : t("No matches for “{q}”.", { q: contextsState.memberSearchedQuery });
  }
  let memberStatus = emptyNote;
  if (contextsState.memberSearching) memberStatus = t("Searching…");
  else if (contextsState.memberBusy) memberStatus = t("Working…");
  return html`
    <form class="project-member-picker" @submit=${(event: SubmitEvent) => void searchProjectMembers(event, context)}>
      <label for="project-member-search">${t("Add people")}</label>
...
function resourceSections(scopeId: string): TemplateResult | typeof nothing {
  ...
  if (!r) {
    return contextsState.resourcesLoading
      ? html`<div class="empty compact">${t("Loading this context's files, crons, apps and skills…")}</div>`
      : html``;
  }
...
async function manageCron(id: string, action: "enable" | "disable" | "delete"): Promise<void> {
  ...
  try {
    ...
    contextsState.resourcesNotice = errMessage(e, t("Couldn't update that cron."));
  } finally {
...
async function deleteScopeSkill(id: string): Promise<void> {
  ...
  try {
    ...
    contextsState.resourcesNotice = errMessage(e, t("Couldn't delete that skill."));
  } finally {
    resourceBusy.delete(key);
    drawContexts();
  }
  ...
  return html`
    <section class="context-panel context-resource-group">
      <div class="context-panel-heading context-resource-heading">
        <h2 class="context-panel-title">${t(label)}</h2>
        <a href=${href}>${t("View all")}</a>
      </div>
    ...
    >
      <form @submit=${(event: SubmitEvent) => void createProject(event)}>
        <div class="project-dialog-head">
          ...
          <div><h2 id="project-dialog-title">${t("New project")}</h2></div>
          <button
            class="project-icon-button"
            type="button"
        ...
        <div class="project-dialog-actions">
          <button class="btn" type="button" @click=${closeCreateProject}>
            ${contextsState.createSaving ? t("Close") : t("Cancel")}
          </button>
          <button class="btn primary" type="submit" ?disabled=${contextsState.createSaving}>
            ${icon(FolderPlus, 15)}<span>${contextsState.createSaving ? t("Creating…") : t("Create project")}</span>
          </button>
        </div>
      </form>
    </dialog>
...
async function createProject(event: SubmitEvent): Promise<void> {
  ...
  if (!name) {
    contextsState.createError = t("Enter a project name.");
    drawContexts();
    queueMicrotask(() => document.querySelector<HTMLInputElement>("#project-name")?.focus());
    return;
  }
  ...
  try {
    ...
    contextsState.createSaving = false;
    contextsState.createError = errMessage(error, t("Couldn't create that project."));
    drawContexts();
    queueMicrotask(() => document.querySelector<HTMLInputElement>("#project-name")?.focus());
  }
}
...
async function searchProjectMembers(event: SubmitEvent, context: CoreContext): Promise<void> {
  ...
  if (query.length < 2) {
    contextsState.memberMatches = [];
    contextsState.memberSearchedQuery = "";
    contextsState.memberError = t("Enter at least two characters.");
    drawContexts();
    return;
  }
...
async function runMemberSearch(context: CoreContext, query: string): Promise<void> {
  ...
  try {
    ...
    if (searchSeq !== memberSearchSeq || contextsState.memberProjectId !== projectId) return;
    contextsState.memberSearchedQuery = "";
    contextsState.memberError = errMessage(error, t("Couldn't search for people."));
  } finally {
...
async function addProjectMember(context: CoreContext, member: DirectoryMatch): Promise<void> {
  ...
  try {
    ...
    if (resetSeq !== contextsResetSeq) return;
    contextsState.memberError = errMessage(error, t("Couldn't add that person."));
  } finally {
...
async function removeProjectMember(context: CoreContext, principalId: string): Promise<void> {
  if (!context.project || contextsState.memberBusy) return;
  const label = memberLabel(context, principalId);
  if (!window.confirm(t("Remove {name} from {project}?", { name: label, project: context.name || t("this project") })))
    return;
  const resetSeq = contextsResetSeq;
  memberSearchSeq++;
  cancelMemberSearchTimer();
  ...
  try {
    ...
    if (resetSeq !== contextsResetSeq) return;
    contextsState.memberError = errMessage(error, t("Couldn't remove that person."));
  } finally {
    if (resetSeq === contextsResetSeq) {
      contextsState.memberBusy = false;
      drawContexts();
...
async function loadScopeResources(scopeId: string): Promise<void> {
  ...
  try {
    ...
    if (stale()) return;
    contextsState.resourcesNotice = errMessage(e, t("Failed to load this context's resources."));
  } finally {
    if (!stale()) {
      contextsState.resourcesLoading = false;
      drawContexts();
  ...
  return html`
    <button class="context-session-row" type="button" @click=${() => void openFromContext(s)}>
      ...
      <span class="context-session-meta">
        ...
        ${readOnly ? html`<span class="ro-lock" title=${t("Read-only — replies happen on the original surface")}>${icon(Lock, 12)}</span>` : nothing}
