...
export function createChatSurface(ctx: ConvCtx): ChatSurface {
    ...
    void (async () => {
      try {
        ...
        if (agent === chatState.agent)
          ctx.composer.state.error = errMessage(err, t("Could not start the conversation."));
      } finally {
        if (agent === chatState.agent) {
          agent.streamFn = normalStreamFn;
    ...
    return html`
      <div class="bg-row ${open ? "open" : ""}">
        ...
        >
          ...
          <span class="bg-row-meta">${t("started {n}", { n: relTime(j.startedAt) })} · ${status}</span>
      ...
      <div class="bg-row watch">
        <div class="bg-row-head static">
          ...
          <span class="bg-row-meta"
            >${t("armed {n}", { n: relTime(w.createdAt) })}${w.lastFiredAt ? t(" · last fired {n}", { n: relTime(w.lastFiredAt) }) : ""} ·
      ...
      <section class="live-work-dock ${expanded ? "expanded" : ""}" aria-live="polite">
        ...
        >
          ...
          <span class="live-work-label"
            >${summary ? summary.label : sheenLabel(`${t("Thinking")}${usedToolsSuffix(work)}`, true)}</span
          >
  ...
  function liveWorkSummary(work: WorkBlock): { icon: IconNode; label: string; detail: string } | null {
    if (work.stale) {
      ...
      return {
        icon: RefreshCw,
        label: verb ? t("{verb} interrupted — resuming…", { verb: t(verb) }) : t("Interrupted — resuming…"),
  ...
  function activeToolSummary(row: ToolRowModel, work: WorkBlock): { icon: IconNode; label: string; detail: string } {
    ...
    return {
      icon: meta.icon,
      label: secs > 0 ? t("{verb} for {secs}s", { verb: t(meta.active), secs }) : t(meta.active),
  ...
  function usedToolsSuffix(work: WorkBlock): string {
    ...
    return n > 0 ? t(" (used {n} tool{s})", { n, s: n === 1 ? "" : "s" }) : "";
  }

  function workLabel(work: WorkBlock): string {
    if (work.stale && (work.status === "thinking" || work.status === "working"))
      return t("Interrupted — resuming…");
  ...
  function segmentSummaryLabel(items: TimelineItem[], work: WorkBlock): string {
    ...
    if (tools > 0) return t("{n} tool call{s}", { n: tools, s: tools === 1 ? "" : "s" });
    const secs = workSeconds(work);
    ...
    switch (tool) {
      ...
      case "history": {
        ...
        return result.count !== undefined
          ? `${q} · ${t("{n} result{s}", { n: result.count, s: result.count === 1 ? "" : "s" })}`
          : q;
      }
      case "memory": {
        ...
        if (result.count !== undefined) {
          detail = `${q} · ${t("{n} result{s}", { n: result.count, s: result.count === 1 ? "" : "s" })}`;
  ...
  function toolRow(row: ToolRowModel, work: WorkBlock, status: WorkBlock["status"], stale = false): TemplateResult {
    ...
    if (kind === "approval") label = t("Approval needed");
      ...
      label = stale ? t("{verb} — interrupted", { verb: t(meta.active) }) : t(meta.active);
